...
const LeaderBoard: React.FC<Board> = ({ leaders, challenge }) => {
  ...
  useEffect (() => {
    (async () => {
      setMonthList(Object.values(leaders?.reduce<Record<any, any>>(
        (acc: any, cur: any) => {
          if (challenge && cur.challenge.id !== challenge) {
            return acc
          } 
          if (cur.period.year !== Number(format(new Date(), 'yyyy')) || cur.period.month !== format(new Date(), 'MMMM')) {
            return acc
          }          
  ...
  return (
    <React.Fragment>
      ...
      <Table size="small">
        ...
        <TableBody>
          {monthList?.sort((a, b) => a.points > b.points? -1 : 1).map((row) => (
